...
import { Tabs, Tab, Button,MenuItem,OutlinedInput, Stack, Checkbox,FormGroup,FormControl, FormControlLabel, useMediaQuery, TextField, FormHelperText } from '@mui/material';
...
import { fetchBusinessApplicantConfirm, fetchBusinessApplyConfirm, fetchBusinessCancel, fetchBusinessChklst, fetchBusinessMgntSave, fetchBusinessTmpSave } from '~/fetches/biz/fetchBusinessAppMgt';
...
const  BusAppMgtDetail = () => {
  ...
  const getApplyConfirm = () => {
    fetchBusinessApplyConfirm(!!receive.state ? receive.state.item.applyId : '').then((res:any) => {
      // setApplyConfirm(Object.values(res))
      setApplyData(res)
  ...
  useEffect(() => {
    ...
    getApplyConfirm()
    if(!!receive.state){
      console.log(receive.state.item.rceptSttusCd)
    }
  },[])
      ...
      <div className='content_body'>
        <Box css={styles.sub_cont02}>
          <div className="content">
            ...
            <TabPanel value={value} index={0}>
              ...
              <Stack direction="row" justifyContent="center" spacing={2} sx={{marginTop: '40px'}} css={styles.btn_next}>
                ...
                {(!!receive.state ? (receive.state.item.rceptSttusCd == 'MAKEUP' || receive.state.item.rceptSttusCd == 'TEMP') : false)
                  ? 
                  <>
                    <CustomButton label={'임시저장'} type={'listBack'} color={'outlined'} onClick={(e)=>{handelBizSend(e,false,'01')}}/>
                    <CustomButton label={'제출'} type={'listBack'} color={'primary'} onClick={(e)=>{handelBizSend(e,true,'01')}}/>
                  </>
                  : null
                }
            ...
            { boxuser == 'INDIVIDUAL' ? !!applcntEnt?
              ...
              <Stack direction="row" justifyContent="center" spacing={2} sx={{marginTop: '40px'}} css={styles.btn_next}>
                ...
                {(!!receive.state ? (receive.state.item.rceptSttusCd == 'MAKEUP' || receive.state.item.rceptSttusCd == 'TEMP') : false)
                  ? 
                  <>
                    <CustomButton label={'임시저장'} type={'listBack'} color={'outlined'} onClick={(e)=>{handelBizSend(e,false,'02')}}/>
                    <CustomButton label={'제출'} type={'listBack'} color={'primary'} onClick={(e)=>{handelBizSend(e,true,'02')}}/>
                  </>
                  : null
                }
              </Stack>
            ...
            <TabPanel value={value} index={2}>
              ...
              { !!applyData ? 
                ...
                <dl>
                  ...
                  <dd>
                    <CustomSelect 
                      value={!!applyData ? applyData.bsnsTypeCd : 'EDC'}
                      data={(userQueries[0].status === 'success') ? userQueries[0].data.list : []}
                    />
                  </dd>
                  <dt>사업기간</dt>
            ...
            :null}
            <Stack direction={'row'} justifyContent={'space-between'} alignItems={'flex-end'}>
              <h4 className="sub_title">첨부파일 <span className="must">*</span> <span className="sub_title_desc">(최대 20MB 첨부 가능)</span></h4>
            </Stack>
            ...
            <Stack direction="row" justifyContent="center" spacing={2} sx={{marginTop: '40px'}} css={styles.btn_next}>
              ...
              {(!!receive.state ? (receive.state.item.rceptSttusCd == 'MAKEUP' || receive.state.item.rceptSttusCd == 'TEMP') : false)
                ? 
                <>
                  <CustomButton label={'임시저장'} type={'listBack'} color={'outlined'} onClick={(e)=>{handelBizSend(e,false,'03')}}/>
                  <CustomButton label={'제출'} type={'listBack'} color={'primary'} onClick={(e)=>{handelBizSend(e,true,'03')}}/>
                </>
                : null
              }
            </Stack>
