...
import { Stack, FormControl, MenuItem, TextField, Checkbox, FormGroup, FormControlLabel,Step, Stepper, StepLabel} from '@mui/material';
    ...
    if(allCheck){
      ...
      if(!!receive.state){
        navigate(`/biz/BusinessAppMgt/BusinessAppConfirmInfo/${receive.state.pblancId}`,{
          state : {chkList : receive.state.chkList, cmpnyTypebox:cmpnyTypebox ,pblancId:receive.state.pblancId,validationBox:boxvalue, title:receive.state.title}})
      }
    }else{
      setIsValid(true);
    }
        ...
        <div className='content_body'>
          <div className="content" style={{paddingBottom:0}}>
            ...
            {boxuser != 'INDIVIDUAL' &&
                    ...
                    <dd>
                      ...
                      <DatePicker
                          ...
                          changeNowDate={(startNewTime: Date | null)=>{
                            setCmpnyTypebox((state:any) => ({ ...state, fondDay:dayjs(startNewTime).format('YYYYMMDD') }));
                          }}
                      />
                      </div>
                    </dd>
              ...
              <NavLink to={`/biz/BusinessAppMgt/BusinessApp/${!!receive.state ? receive.state.pblancId : ''}`}
              state = {{pblancId:!!receive.state ? receive.state.pblancId : ''}}
              >
                <CustomButton label={'이전'} type={'listBack'} color={'outlinedblack'} />
              </NavLink>
                <CustomButton label={'다음'} type={'listBack'} color={'primary'}  onClick={()=>goBusinessAppConfirmInfo()}/> 
              </Stack>
              </>
            }
            {/* [e] 기업&대학회원 */}
              ...
              <>
                ...
                <Box css={styles.table}>
                  <div className="detail_table"> 
                    ...
                    <dl>
                      <dt>성별</dt>
                      <dd>{!!memberData ? memberData.gender : ''}</dd>
                      {/* <dt>내외국인</dt>
                      <dd>{memberData}</dd> */}
                    </dl>
                    <dl>
