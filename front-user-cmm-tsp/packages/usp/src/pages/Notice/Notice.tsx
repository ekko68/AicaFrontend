...
import {TabPanelProps , swiperParams,alwaysCurrencies,Noticeitems, groupId, paramsType, SttusType} from "./NoticeModel";
import * as styles from '~/styles/styles';
import { useTheme, Tab, Tabs, Button, Typography, Box, Paper, useMediaQuery, Stack, FormControl, Select, MenuItem, Card, CardActionArea, Chip, CardMedia, CardContent, List, ListItem, ListItemAvatar, ListItemText, styled, FormGroup, FormControlLabel } from '@mui/material';
...
const Notice = () => {
 ...
 const userQueries = useQueries(
  ...
  // 조회 조건 변경시
  // useEffect(handleClickSearch, [params,params1]);
  ...
  return (
    <div css={styles.container}>
      <Box css={styles.sub_cont01} 
        ...
        <Box className="benner" sx={{transform: direction ? 'translate(0, -81px)' : 'translate(0, -20px)'}}>
          <BreadCrumb />
            <div className="content">
              <div className="txtbox">
                ...
                { modalOpen ? 
                    <NoticeModalMobile
                      open={modalOpen}
                      setModalOpen={(ck:boolean)=>{setModalOpen(ck)}}
        ...
        {/* 정시모집 */}
        <Box css={styles.sub_cont02} sx={{ 
          // transform: direction ? 'translate(0, -81px)' : 'translate(0, -20px)',
          marginTop:( `${height}px`
          // scrollY ? 
          // (isMobile ? `${height - 280}px` : `${height - 350}px`) 
          // :
                        ...
                        <NavLink key={keys} to={`/Notice/Notice/${item.pblancId}`} state={{item:item}}>
                          <ListItem className="noticelist">
                            <ListItemAvatar sx={{ mr: 3 }}>
                              {/* todo.... -> `${process.env.REACT_APP_DOMAIN_PMS_BNET}/pms/api/front/bsns-pblanc/${item.pblancId}/thumbnail`*/}
                              <img src={`${process.env.REACT_APP_DOMAIN_PMS_BNET}/pms/api/front/bsns-pblanc/${item.pblancId}/thumbnail`} onError = {(e: any)=>common.handleImgError(e)}/>
                            </ListItemAvatar>
                            <ListItemText
                                secondary={
                                <React.Fragment>
        ...
        <TabPanel value={value} index={1}>
            ...
            <Box css={styles.sub_list}>
                ...
                <Stack css={styles.bottom_btn} className="mg0">
                  <CustomButton label={'더보기'} type={'full'} color={'item'} onClick={()=>{
                    const itemsPerPage:any = params1.itemsPerPage + 5;
                    setParams1((state) => ({ ...state, itemsPerPage }));
                  }}/>
                </Stack>
                : ""}
              </Box>
