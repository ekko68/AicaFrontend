...
import {ApplyEstmtList} from "~/service/Model";
...
export const EquipmentListView = (props: {
  ...
  const information = ApplyEstimationService.getEstmtList({...pagination})
  const [itemList, setItemList] = useState<ApplyEstmtList[]>([])
  ...
  useEffect(() => {
    if (!information.isLoading && !information.isFetching) {
      if (!!information.data) {
        setItemList(information.data.list.map((m, i) => {
          return {
            key: m.eqpmnId,
            ...m
          }
  ...
  const AddItems = () => {
    if (information.data)
      setPagination({
        ...
        rowsPerPage: pagination.rowsPerPage + information.data.itemsPerPage
      })
  }
  ...
  >
    <Stack sx={{padding: "40px 0 120px", width: '100%', overflow: 'auto'}}>
      <Stack direction={'row'} alignItems={'baseline'} sx={{marginBottom: '20px'}}>
        ...
        <Body3 style={{fontFamily: 'Roboto', fontWeight: 'bold'}} color={Color.azul}>{pagination.rowCount}</Body3>
        <Body3>건</Body3>
      </Stack>
      <Box sx={{
...

      {
        !isLastItem && <Fragment>
              ...
              <CustomButton
                  ...
                  onClick={AddItems/*() => {
            setItemList(itemList.concat([...itemList]))
          }*/}/>
          </Fragment>
      }
    </Stack>
...
const EquipmentItem = (props: { data: ApplyEstmtList, onClick: (id: string) => void }) => {
  return <Stack sx={{minWidth: '380px', height: '359px'}}>
    <img src={props.data.savedFilePath}
         style={{
           width: '100%',
           height: '200px',
