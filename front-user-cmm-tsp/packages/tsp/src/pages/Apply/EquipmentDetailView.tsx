...
export const EquipmentDetailView = (props: {
  ...
  const info = ApplyEstimationService.getEstmtDetailView(id!.toString())
  ...
  if(!info.data) return <Stack>값이 없습니다.</Stack>
  ...
  return <BannerContents
    ...
    <Stack sx={{padding:isMobile ? '40px 0 80px' : '60px 0 120px', overflow:'hidden', width:'100%'}}>
      <Box sx={{borderBottom: '1px solid black', paddingBottom: '20px', textAlign: isMobile ? 'center' : ''}}>
        <H2 bold style={{fontSize: isMobile ? '20px' : '28px', letterSpacing: isMobile ? '-0.8px' : '-1.12px'}}>{info.data.eqpmnNmKorean}</H2>
      </Box>

      <VerticalInterval size={'20px'}/>
      <Stack direction={isMobile ? 'column' : 'row'}>
        <div style={{display: isMobile ? 'flex' : 'unset', justifyContent: isMobile ? 'center' : 'normal'}}>
          <img src={`https://pc.atops.co.kr${info.data.savedFilePath}`} style={{minWidth:isMobile ? '345px' : '380px', height:isMobile ? '345px' : '380px'}}/>
        </div>
        ...
        <Stack justifyContent={'space-around'} width={'100%'}>
          <Box style={{marginBottom:isMobile ? '10px' : '12px'}}>
            ...
            <Body3>{info.data.eqpmnNmEngl}</Body3>
          </Box>
          <Box style={{marginBottom:isMobile ? '10px' : '12px'}}>
            <Body3 weight={500}>{'모델명 : '}</Body3>
            <Body3>{info.data.modelNm}</Body3>
          </Box>
          <Box style={{marginBottom:isMobile ? '10px' : '12px'}}>
            <Body3 weight={500}>{'모델 분류 : '}</Body3>
            <Body3>{info.data.eqpmnClNm}</Body3>
          </Box>
          <Box style={{marginBottom:isMobile ? '10px' : '12px'}}>
            <Body3 weight={500}>{'규격 : '}</Body3>
            <Body3>{info.data.eqpmnStndrd}</Body3>
          </Box>
          <Box style={{marginBottom:isMobile ? '15px' : '20px'}}>
            <Body3 weight={500}>{'전원 : '}</Body3>
            <Body3>{info.data.srcelct}</Body3>
          </Box>
          ...
          <Box style={{marginBottom:isMobile ? '10px' : '12px', marginTop:isMobile ? '15px' : '20px'}}>
            ...
            <Body3>{info.data.mnlAt}</Body3>
          </Box>
          <Box style={{marginBottom:isMobile ? '10px' : '12px'}}>
            <Body3 weight={500}>{'소프트웨어 : '}</Body3>
            <Body3>{info.data.swAt}</Body3>
          </Box>
          <Box style={{marginBottom:isMobile ? '10px' : '12px'}}>
            <Body3 weight={500}>{'설치장소 : '}</Body3>
            <Body3>{info.data.legacyItlpc}</Body3>
          </Box>
          <Box style={{marginBottom:isMobile ? '10px' : '12px'}}>
            <Body3 weight={500}>{'유무료 : '}</Body3>
            <Body3>{info.data.pchrgAt}</Body3>
          </Box>
          <Box>
              <Body3 weight={500} >{'특기사항 : '}</Body3>
              <Body3>{info.data.spcmnt}</Body3>
          </Box>
        </Stack>
      </Stack>
      ...
      <Body3 style={{margin: '12px 0 40px 16px', lineHeight:'28px'}}>{info.data.sumry}</Body3>
      ...
      <Body3 style={{margin: '12px 0 40px 16px', lineHeight:'28px'}}>{info.data.specComposition}</Body3>

      {/*<li style={{fontWeight: 500, fontSize: isMobile ? '16px' : '18px'}}>{'보조기기'}</li>*/}
      <Body2 listItem weight={500} style={{paddingLeft: '7px'}}>{'보조기기'}</Body2>
      <Body3 style={{margin: '12px 0 40px 16px', lineHeight:'28px'}}>{info.data.asstnMhrls}</Body3>

      {/*<li style={{fontWeight: 500, fontSize: isMobile ? '16px' : '18px'}}>{'분야/용도'}</li>*/}
      <Body2 listItem weight={500} style={{paddingLeft: '7px'}}>{'분야/용도'}</Body2>
      <Body3 style={{margin: '12px 0 40px 16px', lineHeight:'28px'}}>{info.data.realmPrpos}</Body3>
