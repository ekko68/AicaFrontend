  ...
  return (
    <Stack width={"100%"} spacing={"40px"}>
      <div style={{
        ...
        <div style={{fontWeight: 900, fontSize: "25px"}}>장비 사용신청이 완료되었습니다.</div>
        <VerticalInterval size={"30px"}/>
      ...
      <Box display={"flex"} justifyContent={"center"} style={{marginBottom: "40px"}}>
        <CustomButton label={<Body2 color={Color.azul}>장비사용관리</Body2>} color={"outlined"}
                      style={{borderRadius: "30px", width:props.isMobile ? '50%' : '168px', height:'60px'}}
                      onClick={() => {
                        navigate('/tsp/Mypage/Equipment');
                      }}
        />
        <Box width={"20px"}/>
