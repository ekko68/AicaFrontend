    ...
    <Stack sx={{padding: isMobile ? '40px 0 80px' : '60px 0 120px', overflow: 'hidden'}}>
      <Box sx={{borderBottom: '1px solid black', paddingBottom: '20px', textAlign: isMobile ? 'center' : ''}}>
        <H2 bold style={{
          ...
          letterSpacing: isMobile ? '-0.8' : '-1.12px'
        }}>{eqpmnData.eqpmnNmKo}</H2>
      </Box>

      <VerticalInterval size={'20px'}/>
      <Stack direction={isMobile ? 'column' : 'row'}>
        <div style={{display: isMobile ? 'flex' : 'unset', justifyContent: isMobile ? 'center' : 'normal'}}>
          <img src={eqpmnData.imgUrl}
               style={{minWidth: isMobile ? '345px' : '380px', height: isMobile ? '345px' : '380px'}}/>
        </div>
      ...
      <Box sx={{display: 'flex', justifyContent: 'center'}}>
        <CustomButton label={'목록'} type={'listBack'}
                      style={{
        ...
        <CustomButton
          ...
          type={'frontNomal'}
          style={{
            width: isMobile ? '167px' : '202px',
            fontSize: isMobile ? '16px' : '18px',
            letterSpacing: isMobile ? '-0.68' : '-0.74',
            padding: '16px 18px 17px'
          }}
          onClick={() => {
            addModal({open: true, content: '로그인 기능 구현중입니다.', isDist: true})
          }}/>
      </Box>
    </Stack>
  </BannerContents>
