...
const MyPageEquipment = () => {
  ...
  return <BannerContents
    title={"장비사용 관리"} subTitle={"장비 신청정보를 조회 및 수정하고, 현재 사용상태를 확인할 수 있습니다."}
    placeholder={'장비 신청 정보를 조회해 보세요.'}
    detail={{
      column1Title: '신청일',
      column2Title: '신청상태',
      condition: ['신청', '보완요청', '승인', '반려', '신청취소', '사용중', '사용종료'],
    ...
    <Stack sx={{padding: "40px 0 120px", width: '100%', overflow: 'auto'}}>
      <Stack
        ...
        <Stack direction={'row'} alignItems={'baseline'}>
          ...
          <Body4 bold color={Color.azul}>{10}</Body4>
          <Body3>건</Body3>
        </Stack>
            ...
            title={
              <Stack>
                <Body4 weight={500}
                  ...
                  신청 {!isDesktop && <VerticalInterval size={'9px'}/>}
                ...
                <Body4 weight={500}
                       style={{paddingLeft: isDesktop ? '6px' : '8px', marginBottom: isDesktop ? '6px' : '16px'}}
                       listItem color={TypeColor('사용종료')}>
                  사용종료 {!isDesktop && <VerticalInterval size={'9px'}/>}
                  <Body4 style={{marginLeft: isDesktop ? '5px' : 0}}
                         color={Color.warm_gray}>{`사용종료 상태`}</Body4>
                </Body4>
              </Stack>
            }
          >
          ...
          return <Fragment key={i}>
            <Button sx={{justifyContent: 'flex-start'}}
                    onClick={(e: React.MouseEvent<HTMLElement>) => {
                      navigate(`/tsp/Mypage/Equipment/${i}`)
                    }}>
              <EquipmentManageItem data={m}/>
            </Button>
...
const EquipmentManageItem = (props: { data: UsingEquipmentData }) => {
  const {isDesktop} = useGlobalConfigStore()
...
const TypeColor = (type: EquipmentType) => {
  switch (type) {
    case "신청":
      return Color.azul
    case "보완요청":
...
type EquipmentType = '신청' | '보완요청' | '승인' | '반려' | '신청취소' | '사용중' | '사용종료'

interface UsingEquipmentData {
  title: string
  startTime: Date,
  endTime: Date,
  createDt: Date,
...
export default MyPageEquipment