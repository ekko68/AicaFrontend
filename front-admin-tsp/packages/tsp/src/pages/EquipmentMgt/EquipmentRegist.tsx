...
import {Box, FormControl, Stack, Table, TableCell, TableContainer, TableRow, TextField} from "@mui/material";
import {EquipmentCategoryService} from "~/service/EquipmentMgt/EquipmentCategoryService";
import {
  DiscountData, EqpmnEstmtDetailData, EqpmnUseReqstDetail,
      ...
      <SubContents title={"상세설정"}>
        <TableContainer style={{borderTop: "1px solid #d7dae6"}}>
          <Table>
            <TableRow>
              <TableTextFieldCell
                division label={"1시간 사용료"} required
                defaultLabel={""}
                additionDirection={'row'}
                additionContent={<Fragment>
                  <HorizontalInterval size={'15px'}/>
                ...
                setReq((state) => ({...req!, rntfeeHour: Number(text)}))
              }}
              />
              <TableDateTermCell
