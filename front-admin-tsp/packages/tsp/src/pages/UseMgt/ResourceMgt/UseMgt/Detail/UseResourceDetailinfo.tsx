  ...
  return <Fragment>
        ...
        <TableContainer style={{borderTop: "1px solid #d7dae6"}}>
          <Table>
            <TableBody>
              ...
              <TableRow>
                <TableTextCell
                  division title={"사용상태"} label={toCommtCodeName(commtCode, 'EQPMN_RESOURCE_USAGE_ST', state.useSttus)}
                  thWidth={"20%"} tdWidth={"30%"}
                />
                <TableTextCell
    ...
    <Stack flexDirection={"row"} justifyContent={"space-between"} style={{width: "100%"}}>
      ...
      <Stack flexDirection={"row"}>
        {
          (state.useSttus == 'USE') && <CustomButton
                label={"반환요청"}
                type={"largeList"}
                onClick={async () => {
                  if (resourceDetail) {
        ...
        {
          (state.useSttus == 'USE' || state.useSttus == 'REQ_RETURN') && <CustomButton
                label={"반환완료"}
                type={"largeList"}
                onClick={async () => {
                  if (resourceDetail) {
        ...
        {
          state.useSttus == 'WAITING' && <CustomButton
          label={"승인취소"}
          type={"largeList"}
          onClick={async () => {
            if (resourceDetail) {
