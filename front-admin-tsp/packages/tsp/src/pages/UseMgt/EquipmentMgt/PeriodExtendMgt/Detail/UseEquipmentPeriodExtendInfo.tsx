...
import {DiscountData, EqpmnEstmtDetailData, EqpmnExtndDetail, PaymentState, StatusState} from "~/service/Model";
...
export const UseEquipmentPeriodExtendInfo = () => {
  ...
  const [dataDscnt] = useState<DiscountData[]>([{dscntId: '0', dscntRate: 0, usgstt: true, dscntResn: '없음'}])
        ...
        <Table>
          <TableBody>
            ...
            <TableRow>
              ...
              <TableTextCell title={"AI 집적단지 사업참여 여부"} label={state.partcptnAt === 't' ? '참여' : '미참여'}
                             thWidth={"13%"}/>
          ...
          <TableBody>
            ...
            <TableRow>
              <TableTextCell
                ...
                label={`${Math.floor(state.rntfeeHour).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') || 0}원`}
                thWidth={"13%"} tdWidth={"21%"}/>
              <TableTextCell
                division title={"사용금액"}
                label={`${Math.floor(state.usgtm * state.rntfeeHour).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') || 0}원`}
                thWidth={"13%"} tdWidth={"21%"}/>
              <TableTextCell
            ...
            <TableRow>
              {sttusDiv
                ...
                <TableSelectCell
                  label={"할인사유"}
                  defaultLabel={state.dscntResn === null ? '없음.' : `${state.dscntResn} (${state.dscntRate}%)`}
            ...
            <TableRow>
              <TableTextCell
                division title={"할인금액"}
                label={`${Math.floor(((state.usgtm * state.rntfeeHour) * state.dscntRate / 100)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') || 0}원`}
                thWidth={"13%"} tdWidth={"21"}/>
              <TableTextCell
                title={"할인적용금액"}
                label={`${Math.floor(((state.usgtm * state.rntfeeHour) - (state.usgtm * state.rntfeeHour) * state.dscntRate / 100)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') || 0}원`}
                thWidth={"13%"} tdSpan={5}/>
            </TableRow>
          </TableBody>
        </Table>
