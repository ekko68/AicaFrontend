...
export const UseEquipmentExtendPeriodMgt: React.FC<{
  ...
  if (information.isLoading || !information.data) return <LoadingProgress/>
      ...
      /> : <TableComponents<EqpmnUseExtendPeriod>
        ...
        tableCell={(data) => {
          ...
          return <Fragment>
            ...
            <TableCell key={"period-" + data.key}
                       align={"center"}>{`${toTimeFormat(data.useBeginDt)} ~ ${toTimeFormat(data.useEndDt)}`}</TableCell>
            <TableCell key={"usgtm-" + data.key} align={"center"}>{`${data.usgtm || 0}시간`}</TableCell>
            <TableCell key={"rntfeeHour-" + data.key}
                       align={"center"}>{`${data.rntfeeHour.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}원`}</TableCell>
            <TableCell key={"rntfee-" + data.key}
                       align={"center"}>{`${data.rntfee.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}원`}</TableCell>
            <TableCell key={"dscntAmount-" + data.key}
                       align={"center"}>{`${data.dscntAmount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') || 0}원`}</TableCell>
            <TableCell key={"pymntMth-" + data.key} align={"center"}>{pymntMth}</TableCell>
