    ...
    tableCell={(data) => {
      ...
      return (
        <Fragment>
          ...
          <TableCell sx={{textAlign: 'center'}} key={"tkoutAt-" + data.key}>{data?.tkoutAt ? '반출' : '미반출'}</TableCell>
          <TableCell sx={{textAlign: 'center'}} key={"tkinAt-" + data.key}>{data?.tkinAt ? '반입완료' : '반입전'}</TableCell>
