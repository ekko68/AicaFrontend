...
import { EquipmentClassifyService } from '~/service/EquipmentClassifyService';
...
const axiosTest = async (data: {
  parentId: string;
  req: EquipmentClassifyRequest[];
}) => {
  return EquipmentClassifyService.setTest(data.parentId, data.req);
};

export const EquipmentClassify = () => {
  const { data } = EquipmentClassifyService.getRoot();
  const { tableRows, setCategory } = useEquipmentClassifyStore();
  const [root, setRoot] =
    useState<EquipmentClassifyData | undefined>(undefined);
  // const deleteMutation = useMutation(axiosTest);

  // deleteMutation.mutate()

  const sort = (
    data: EquipmentClassifyData,
    ...
    if (equipment.eqpmnClfcId == 'ROOT') {
      ...
      if (
        data.child.filter((f) => f.eqpmnClfcId == equipment.eqpmnClfcId)
          .length == 0
      )
  ...
  const flatten = (
    dest: EquipmentClassifyData[],
    source: EquipmentClassifyData[]
  ) => {
    source.map((m) => {
      dest.push({ ...m, child: [] });
      ...
      {tableRows && (
        <RightTableView
          onPresSave={async (
            ...
            try {
              const editEquipment = await EquipmentClassifyService.setCategory(
                parentId,
                req
              );
              const updateData: EquipmentClassifyData[] = editEquipment.map(
                (m) => {
                  return { ...m, child: [] };
                }
              );
              ...
              const flatRoot: EquipmentClassifyData[] = [
                { ...root!, child: [] },
              ];
              flatten(flatRoot, root!.child);

              const newData = updateData.filter(
                (f) => !flatRoot.find((d) => d.eqpmnClfcId == f.eqpmnClfcId)
              );
              const edit = [
                ...flatRoot.map((m) => {
                  const data = updateData.find(
                    (f) => f.eqpmnClfcId == m.eqpmnClfcId
                  );
                  if (data) return data;
                  else return m;
                }),
          ...
          onPressDelete={async (
            ...
            try {
              const deleteEquipment =
                await EquipmentClassifyService.deleteCategory(parentId, req);
              ...
              const classify = flatRoot
                .filter((f) => !deleteIds.includes(f.eqpmnClfcId))
                .reduce(sort);
              setRoot(classify);
              setCategory(
                findCategory(parentId, classify) || classify.child[0]
              );
            } catch (e) {
              console.warn(e);
            }
  ...
  return (
    ...
    >
      <TreeItem
        ...
        nodeId={'root'}
        label={
          <div onClick={() => props.onClickTree(props.root!)}>{'장비분류'}</div>
        }
      >
        {props.root &&
          props.root.child.map((m, i) => {
            return (
              <Item
                key={i}
                depth={1}
      ...
      <DataTable
        ...
        onSelectionModelChange={(selectionModel: GridSelectionModel) => {
          const selected = category?.child.filter((f) => {
            if (selectionModel.includes(f.eqpmnClfcId!)) return f;
          });
          if (!!selected) setSelectData(selected);
        }}
        onCellEditCommit={(params: GridCellEditCommitParams) => {
          setRow(
            tableRows.map((m) => {
              if (m.id == params.id) return { ...m, title: params.value };
              else return m;
            })
          );
        }}
