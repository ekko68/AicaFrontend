...
import {공지상세정보, 관련주소사이트, 관리자공지} from "~/pages/OperationMgt/CustomerSupportMgt/Model/Model";

export const ManagerNoticeMgtDetail = () => {
  ...
  return <Stack id={'ManagerNotice'} width={'100%'}>
    <TitleContents title={"관리자공지사항 상세"}>
          ...
          <Stack flexDirection={"row"} justifyContent={"space-between"} style={{width: "100%", marginTop: "40px"}}>
            <Stack>
              <CustomButton
                ...
                onClick={() => {
                  navigate('/OperationMgt/CustomerSupportMgt/ManagerNoticeMgt')
                }}
              />
            </Stack>
            <Stack direction={'row'}>
              <CustomButton
                ...
                type={"largeList"}
                onClick={async () => {
                  //   const result = await ManagerNoticeMgtService.delete(id!)
                  //   if (result.success) {
                       addModal({
              ...
              <CustomButton
                ...
                type={"largeList"}
                onClick={() => {
                  if (checkValidity('ManagerNotice')) {
...
export default ManagerNoticeMgtDetail