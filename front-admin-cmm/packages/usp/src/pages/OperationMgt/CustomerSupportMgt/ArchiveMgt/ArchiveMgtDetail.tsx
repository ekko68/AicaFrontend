...
export const ArchiveMgtDetail = () => {
  ...
  return <Stack id={'Archive'} width={'100%'}>
    <TitleContents title={"자료 상세"}>
          ...
          <Stack flexDirection={"row"} justifyContent={"space-between"} style={{width: "100%", marginTop: "40px"}}>
            <Stack>
              <CustomButton
                ...
                onClick={() => {
                  navigate('/OperationMgt/CustomerSupportMgt/ArchiveMgt')
                }}
              />
            </Stack>
            <Stack direction={'row'}>
              <CustomButton
                ...
                type={"largeList"}
                onClick={async () => {
                  //   const result = await ArchiveMgtService.delete(id!)
                  //   if (result.success) {
                       addModal({
              ...
              <CustomButton
                ...
                type={"largeList"}
                onClick={() => {
                  if (checkValidity('Archive')) {
                    addModal({open: true, isDist: true, type: 'normal', content: '저장되었습니다.'})
                    navigate('/OperationMgt/CustomerSupportMgt/ArchiveMgt/1158')
                  }

                  // const result = await ArchiveMgtService.deleteEquipment(id!)
                  // if (result.success) {
                  //   addModal({
...
export default ArchiveMgtDetail