...
import {dummyFreAskList, 자주묻는질문, SearchParam} from "~/pages/OperationMgt/CustomerSupportMgt/Model/Model";
...
const ListView: React.FC<{
  ...
  const [rowList, setRowList] = useState<WithCustomRowData<자주묻는질문>[]>(dummyFreAskList);
  const navigation = useNavigate();
  // const FreAskList = FrequentlyAskedQutService.getList({...pagination, ...props.searchParam});
  // useEffect(() => {
  //   if (!FreAskList.isLoading || !FreAskList.isFetching) {
  //     if (!!FreAskList.data) {
  //       setRowList(FreAskList.data.list.map((m,) => {
  //         return {
  //           key: m.id,
  //           ...m,
  //         }
  ...
  return <TableComponents<자주묻는질문>
    showTotal
    // isLoading={FreAskList.isLoading || FreAskList.isFetching}
    rightContent={<Stack flexDirection={"row"}>
      <CustomButton
        ...
        color={"list"}
        onClick={async () => {
          navigation('/OperationMgt/CustomerSupportMgt/FrequentlyAskedQut/FrequentlyAskedQutRegi')
        }}
      />
    </Stack>}
    headCells={headCells}
    ...
    handleClick={() => {
      navigation('/OperationMgt/CustomerSupportMgt/FrequentlyAskedQut/FrequentlyAskedQutDetail');
    }}
    tableCell={(data:WithCustomRowData<자주묻는질문>,i) => {
      return <Fragment>
...
const headCells: CustomHeadCell<자주묻는질문 & {count: number}>[] = [
  {
    id: 'count',
    align: 'center',
    label: '번호',
