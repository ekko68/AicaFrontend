...
import {홈팝업창상세} from "~/pages/OperationMgt/SiteMgt/Model";
...
export const HomePopupWindowDetailInfo = () => {
  const [state, setState] = useState<홈팝업창상세>(tempData);
  ...
  return <Stack spacing={'40px'} component={'form'} id={'popupWindowDetail'}>
      ...
      <TableContainer style={{borderTop: "1px solid #d7dae6"}}>
        <Table>
          <TableBody>
            ...
            <TableRow>
              <TableTextFieldCell division label={'팝업창 폭'} required thWidth={'12%'} onChange={(text) => {
                setPopupWidth(Number(text))
                setState({...state,팝업창폭:Number(text)})
              }}/>
              <TableTextFieldCell label={'팝업창 높이'} required thWidth={'12%'} onChange={(text) => {
    ...
    <Stack flexDirection={"row"} justifyContent={"space-between"} style={{width: "100%"}}>
      ...
      <Stack flexDirection={'row'}>
        ...
        <CustomButton
          ...
          onClick={() => {
            if (checkValidity('popupWindowDetail') && selectedImage) {
    ...
    <Box height={'100%'} width={'100%'} textAlign={'center'}>
      {
        props.image ?
          <img src={URL.createObjectURL(props.image)}
               height={props.height} width={props.width}/> : '이미지를 등록하세요'
      }
    </Box>
  </ModalComponents>
}

const tempData: 홈팝업창상세 = {
  등록일: Date.now(),
  전시여부: '전시',
  팝업명: '팝업명이 출력됩니다.',
  이동경로: 'URL이 출력됩니다.',
