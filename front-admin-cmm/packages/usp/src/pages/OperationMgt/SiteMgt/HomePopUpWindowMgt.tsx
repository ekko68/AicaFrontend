...
import {검색파라미터, 홈팝업창목록조회} from "~/pages/OperationMgt/SiteMgt/Model";

function HomePopUpWindowMgt() {
  const [searchParam, setSearchParam] = useState<검색파라미터>()
  return <TitleContents title={'홈팝업창관리'}>
    ...
    <TableContainer>
      <SearchTable>
        <TableBody>
          ...
          <TableRow>
            <TableTextFieldCell thWidth={"12%"} tdWidth={"38%"} tdSpan={3} label={'팝업명'} onChange={(text: string) => {
              setSearchParam({...searchParam, 팝업명: text})
            }}/>
          </TableRow>
          <TableRow>
...
const ListView: React.FC<{
  ...
  const [rowList, setRowList] = useState<WithCustomRowData<홈팝업창목록조회>[]>(tempData);
  ...
  return <TableComponents<홈팝업창목록조회>
    showTotal
    ...
    rightContent={<Stack flexDirection={"row"}>
      <CustomButton label={'등록'} type={"small"} color={'list'} onClick={() => {
        navigation('/OperationMgt/SiteMgt/HomePopUpWindowMgt/ApplyPopUp')
      }}/>
    ...
    handleClick={(key: string) => {
      navigation('/OperationMgt/SiteMgt/HomePopUpWindowMgt/' + key);
    }}
    tableCell={(data) => {
      /*let reqstSttus = ''
      let pymntMth = ''
      ...
      return (
        <Fragment>
          <TableCell sx={{textAlign: 'center'}} key={"팝업명-" + data.key}>{data.팝업명}</TableCell>
...
const headCells: CustomHeadCell<홈팝업창목록조회>[] = [
  {
    id: '팝업명',
    align: 'center',
    label: '팝업명',
...
const tempData: WithCustomRowData<홈팝업창목록조회>[] =
  [{
    key: 'id-1234',
    팝업명: '팝업명 출력',
    전시시작일: Date.now(),
