...
import {Box, CircularProgress, Stack, Table, TableBody, TableContainer, TableRow} from "@mui/material";
...
export const EquipmentMgtInfo = () => {
  ...
  useEffect(() => {
    if (!mgt.isLoading && !mgt.isFetching) {
      if (!!mgt.data) {
        mgtInfo.setMgtInfoData(mgt.data)
        if (!req) {
          setReq(mgt.data)
        }
      }
          ...
          <Stack flexDirection={"row"}>
            {
              ...
              <CustomButton type={"small"} color={"list"} label={"수리내역수정"} onClick={async () => {
            ...
            <HorizontalInterval size={"10px"}/>
            {
              mgtInfo.mgtInfoData.repairParam && mgtInfo.mgtInfoData.eqpmnSttus == 'REPAIR' ?
                <CustomButton type={"small"} color={"list"} label={"수리완료"} onClick={async () => {
                  if (isDisuseCheck(addModal, mgtInfo.mgtInfoData?.disuseAt)) return
            ...
            <TableBody>
              <TableRow>
                <TableTextCell
                  ...
                  label={repairParam ? `${dayFormat(repairParam.manageBeginDt)} ~ ${dayFormat(repairParam.manageEndDt)}` : ""}
                  thWidth={"13%"} tdWidth={"21%"}
                />
                <TableTextCell
                  title={"수리 내역"} label={repairParam ? repairParam.manageResn : ""}
                  thWidth={"13%"}
                />
              </TableRow>
            </TableBody>
        ...
        rightContent={<Fragment>
          {
            ...
            <CustomButton type={"small"} color={"list"} label={"점검완료"} onClick={async () => {
              const endCheck = await EquipmentInformationService.getEquipmentsEndCheck(id!, 'INSPECT')
              mgtInfo.setCorrectData(endCheck);
              ...
              <TableRow>
                <TableRadioCell
                  ...
                  onClick={(selected: string) => {
                    setReq((state) => ({
                      ...state!,
                      crrcTarget: selected == "예"
                    }))
                  }}
                />
                <TableSelectCell
                  division label={"교정주기"}
                  // disable={req?.crrcTargetAt ? false : true}
                  selectLabel={["1년", "15일", "1달"]}
                  thWidth={"13%"} tdWidth={"21%"}
                  onClick={(selected: string) => {
                    setReq((state) => ({
                      ...state!,
