        ...
        <TableBody>
          ...
          <TableRow>
            <TableSelectTextFieldCell
              ...
              selectLabel={["장비명", "자산번호", "사업자명/이름", "접수번호"]}
              defaultLabel={searchText}
              onClick={(selected: string) => {
                setSearchKeyword(selected)
                setSearchText("")
                setSearchParam({...searchParam, assetsNo: '', modelNm: '', entrprsNm: '', rceptNo: ''})
              }}
...
const ListView: React.FC<{
  ...
  return <TableComponents<EqpmnUseReqstList>
    ...
    handleClick={(key: string) => {
      navigation('/tsp_admin/UseMgt/EquipmentMgt/ApplyMgt/' + key);
    }}
    tableCell={(data) => {
      let reqstSttus = ''
      let pymntMth = ''
      ...
      return (
        <Fragment>
          {/*<TableCell key={"number-" + data.key}>{index + 1}</TableCell>*/}
