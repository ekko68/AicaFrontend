...
import {dayFormat} from '~/../../shared/src/utils/stringUtils';
import {
  UseResourceMgtData,
        ...
        <TableBody>
          <TableRow>
            <TableSelectCell
              ...
              thWidth={"12%"} tdSpan={3}
              selectLabel={["전체", "대기중", "사용중", "반환요청", "반환완료"]}
              defaultLabel={"전체"}
              medium
              onClick={(selected: string) => {
                setUseResourceSearchParam({...useResourceSearchParam, useSttus: selected})
              }}/>
          </TableRow>
          <TableRow>    
            <TableDateTermCell
...
> = props => {
  ...
  return <TableComponents<UseResourceMgtData>
      ...
      handleClick={(key: string) => {
        navigation('/tsp_admin/UseMgt/ResourceMgt/UseMgt/' + key);
      }}
      tableCell={(data) => {
        return (
            data ? <Fragment>
              <TableCell key={"useSttus"} sx={{paddingLeft: "30px"}}>{(data.useSttus)}</TableCell>
              <TableCell key={"mberDiv"}>{data.mberDiv}</TableCell>
              <TableCell key={"entrprsNm"}>{data.entrprsNm}/{data.userNm}</TableCell>
