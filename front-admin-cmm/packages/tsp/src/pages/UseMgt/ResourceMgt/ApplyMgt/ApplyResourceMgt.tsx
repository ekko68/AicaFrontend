...
import {toStringFullDayFormat} from '~/../../shared/src/utils/stringUtils';
import {ApplyResourceMgtData, ApplyResourceSearchParam} from '~/service/Model';
        ...
        <TableBody>
          <TableRow>
            <TableSelectCell
              ...
              thWidth={"12%"} tdSpan={3}
              selectLabel={["전체", "신청", "보완요청", "승인", "반려", "신청취소"]}
              defaultLabel={"전체"}
              medium
              onClick={(selected: string) => {
                setApplyResourceSearchParam({...applyResourceSearchParam, reqstSttus: selected})
              }}/>
          </TableRow>
          <TableRow>
            <TableDateTermCell
              ...
              onChange={(beginTime, endTime) => {
                setApplyResourceSearchParam((e) => ({
                  ...e!,
                  creatBeginDt: beginTime.getTime(),
                  creatEndDt: endTime.getTime(),
                }))
              }}
            />
...
const ListView:React.FC<{
  ...
  return <TableComponents<ApplyResourceMgtData>
    ...
    handleClick={(key: string) => {
      navigation('/tsp_admin/UseMgt/ResourceMgt/ApplyMgt/' + key);
    }}
    tableCell={(data) => {
      return (
        data ? <Fragment>
          <TableCell key={"reqstSttus"} sx={{paddingLeft: "30px"}}>{(data.reqstSttus)}</TableCell>
          <TableCell key={"mberDiv"}>{data.mberDiv}</TableCell>
