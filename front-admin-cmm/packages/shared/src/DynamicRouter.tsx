...
import {WithListResponse} from "./utils/Model";
...
export function DynamicRouter({addRoutes, portalType}: IDynamicRouter) {
  ...
  useEffect(() => {
    if (!portal.isLoading && !portal.isFetching){
      if (!!portal.data){
        ...
        client.setQueryData<RouteData[]>('record', record)
        client.setQueryData<RouteData[]>('routes', routes)
  ...
  const routes = client.getQueryData('routes')
...
const adaptor = (routes: any) => {
  return routes.map((route: any) => {
    ...
    return {
      label,
      path: `${process.env.REACT_APP_PROJECT}` == 'usp'? path : "/tsp_admin"+path,
      element: (
        <Loader
          route={middleware({
