...
import {
  fetchBusinessApplyData,
  fetchBusinessApplyDataSave,
  fetchBusinessApplyDataSubmit
} from "~/fetches/fetchBusiness";
...
const BusinessAppConfirmInfo = () => {
  ...
  const getList = () => {
    fetchBusinessApplyData(receive.state.pblancId).then((res: any) => {
      if (!!receive.state && !!receive.state.applyTask){
        setApplyTask(receive.state.applyTask)
      }else {
        setApplyTask((state: any) => ({
          ...state,
          taskTypeCd: res.taskTypeCd,
          applyRealmId: res.applyRealmList[0]?.applyRealmId
        }))
      }

      setApplyData(res)
      if (!!receive.state){
        if (!!receive.state.taskPartcptsList) setTaskPartcptsList(receive.state.taskPartcptsList)
        if (!!receive.state.uploadFileList) setFiles(receive.state.uploadFileList)
      }
    }).catch((e) => {
        ...
        <div className='content_body'>
          <div className="content" style={{paddingBottom: 0}}>
            ...
            <table className="tableDefault type5 pc">
              ...
              <tbody>
              <ConfirmInfoTable checkList={select} change={changeCheck} data={taskPartcptsList}
            ...
            {applyData && applyData.bsnsTypeNm === "교육사업" && receive.state.cmpnyTypebox === null ?
              ...
              : null}
            <Stack direction={'row'} justifyContent={'space-between'} alignItems={'flex-end'}>
              <h4 className="sub_title">파일첨부<span className="must">*</span> <span className="sub_title_desc">(최대 20MB 첨부 가능)</span>
              </h4>
            </Stack>
            ...
            <Box css={styles.agreement}>
              <FormGroup>
                <FormControlLabel
                  control={<Checkbox checked={allCheck1} onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
            ...
            <Stack direction="row" justifyContent="center" spacing={2} sx={{marginTop: '40px'}} css={styles.btn_next}>
              <NavLink
                ...
                state={{
                  ...
                  cmpnyTypebox: receive.state.cmpnyTypebox,
              ...
              <CustomButton label={'임시저장'} type={'listBack'} color={'outlined'} onClick={(e: any) => {
                send(e, 0)
              }}/>
              <CustomButton label={'제출'} type={'listBack'} color={'primary'} onClick={(e: any) => {
                send(e, 1)
              }}/>
            </Stack>
          </div>
