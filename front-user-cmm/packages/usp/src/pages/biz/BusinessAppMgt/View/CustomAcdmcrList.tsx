...
export const CustomAcdmcrList: React.FC<{
  codeList: codeType[]
  data: UsptAcdmcr
  setData: (update: UsptAcdmcr) => void
  ...
  return (
    <tr>
      ...
      <td><OutlinedInput size="small" className="ipt_tp01" sx={{width: '100%'}} value={props.data.schulNm}
                         name='schulNm' onChange={handleOnChange}/></td>
      <td><OutlinedInput size="small" className="ipt_tp01" sx={{width: '100%'}} value={props.data.major} name='major'
                         onChange={handleOnChange}/></td>
      <td>
        <FormControl fullWidth>
          <Select
            value={props.data.grdtnDivCd}
            name='grdtnDivCd'
            onChange={(e) => {
              props.setData({
                ...props.data,
