...
function TenantCompPerforMgt() {
  ...
  const getRsltId = () => {
    if (!!movin.data) {
      const data = {
        mvnId: movin.data?.mvnId,
        rsltYm: year + month,
      }
      fetchRsltIdGet(data).then((res: any) => {
        console.log('getRsltId - ' + JSON.stringify(res))
  ...
  const getRsltId2 = () => {
    if (!!movin.data) {
      ...
      fetchRsltIdGet(data).then((res: any) => {
        console.log('getRsltId2 - ' + JSON.stringify(res))
          ...
          <div className="content">
            <div className="ai_startup">
                    ...
                    <dl>
                      ...
                      <dd>
                        <div className='select_set'>
                          ...
                          <CustomSelect
                            value={month}
                            data={[{code: "01", codeNm: "01"}, {code: "02", codeNm: "02"}, {
                              code: "03",
                              codeNm: "03"
                            }, {code: "04", codeNm: "04"}, {code: "05", codeNm: "05"}, {
                              code: "06",
                              codeNm: "06"
                            }, {code: "07", codeNm: "07"}, {code: "08", codeNm: "08"}, {
                              code: "09",
                              codeNm: "09"
                            }, {code: "10", codeNm: "10"}, {code: "11", codeNm: "11"}, {code: "12", codeNm: "12"}]}
                            onClick={(selected) => {
                              setMonth(selected)
                            }}/>
                        </div>
              ...
              <TabPanel value={value} index={1}>
                ...
                <h4
                  className='tbl_title'>{data2 && data2.rsltIdxIemList[0] ? data2.rsltIdxIemList[0].rsltIdxNm : null}</h4>
                        ...
                        <tr>
                          ...
                          {rsltIdxIemCnList2?.map((res: any) => (
                            stdIemNm?.map((k: any) => (
                              res.stdIemNm === k && res.detailIemNm === item ?
                                <td className='tar'>
                                  {inputPriceFormat(res.rsltIdxIemCn)}<span
                                  className='unit_space'></span>{checkIemUnit(res.iemUnitCd)}
                                </td>
                                : null
                            ))
                          ))}
                ...
                {rsltIdxIemCnList2 && Array.isArray(detailIemNm2) && detailIemNm2.length === 0 ?
                  <h4
                    className='tbl_title'>{data2 && data2.rsltIdxIemList2 ? data2.rsltIdxIemList2[0].rsltIdxNm : null}</h4>
                  : null}
                {rsltIdxIemCnList2 && Array.isArray(detailIemNm2) && detailIemNm2.length === 0 ?
                  <Box css={styles.table}>
                        ...
                        {rsltIdxIemCnList2.map((item: any, i: number) => (
                          i * 2 < rsltIdxIemCnList2.length ?
                            <dl>
                              ...
                              <dd className='tar'>{inputPriceFormat(rsltIdxIemCnList2[i * 2].rsltIdxIemCn)}<span
                                className='unit_space'></span>{checkIemUnit(rsltIdxIemCnList2[i * 2].iemUnitCd)}</dd>
                              <dt>{rsltIdxIemCnList2[i * 2 + 1].detailIemNm}</dt>
                              <dd className='tar'>{inputPriceFormat(rsltIdxIemCnList2[i * 2 + 1].rsltIdxIemCn)}<span
                                className='unit_space'></span>{checkIemUnit(rsltIdxIemCnList2[i * 2 + 1].iemUnitCd)}
                              </dd>
                            </dl>
                            : null
                        ))}
                ...
                {rsltIdxIemCnList2 && (rsltIdxIemCnList2[0].iemUnitCd === "TEXT" || rsltIdxIemCnList2[0].iemUnitCd === "DSCRP") ?
                  <>
                    <h4
                      className='tbl_title'>{data2 && data2.rsltIdxIemList2[0] ? data2.rsltIdxIemList2[0].rsltIdxNm : null}</h4>
                    <Box css={styles.table}>
                      {rsltIdxIemCnList2 && rsltIdxIemCnList2[0].stdIemNm ?
                        <div className="detail_table">
                          <dl className='horz'>
                ...
                {rsltId2 ?
                  <Stack flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'}
                         className='tbl_title' sx={{marginBottom: '10px'}}>
                    <h4>파일첨부</h4>
