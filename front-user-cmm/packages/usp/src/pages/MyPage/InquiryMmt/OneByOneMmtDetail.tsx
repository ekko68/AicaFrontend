...
import { fetchOneByOneMmtDetail} from '~/fetches';
...
/* 
  작성일    :   2022/06/01
  화면명    :   마이페이지/ -> 1:1문의 상세페이지
  회면ID    :   UI-USP-FRN-0350201
  화면/개발 :   Seongeonjoo / navycui
*/
export default  function OneByOneMmtDetail () {
  ...
  // 상세 조회
  const { data } = useQuery(["getDetail",questId], async () => await fetchOneByOneMmtDetail(questId,`${process.env.REACT_APP_USP_PERSNAL}`));
  ...
  useEffect(() => {
   ...
   //이전글
  const isPre = (element:any) => {
    if(element.rn === rowNum-1){
      return true;
    }
  }
  const pre:any = lists.filter(isPre);
  console.log('preprepreprepreprepre',pre)
  //다음글
  const isNext = (element:any) => {
    if(element.rn === rowNum+1){
      return true;
    }
  }
  const next:any = lists.filter(isNext); 
  console.log('nextnextnextnextnextnext',next)
  // const attachmentList = data?.attachmentList;

  // 첨부파일 다운로드
  const download = async (attachmentId: any) =>{
    fetchDownload(`${process.env.REACT_APP_DOMAIN_COMMON_BNET}/common/api/qna/${process.env.REACT_APP_USP_PERSNAL}/quests/${questId}/attachments/${attachmentId}`)
    .then()
    .catch((e)=>{
        ...
        <div className="content">
          ...
          <div css={styles.detal_list}>
            ...
            <Box css={styles.detal_txtBox}>
              <Stack direction="row" spacing={1} component="span" className="detail_chip">
                {!!data ? 
                  <Chip label={
                ...
                {
                  !!data ? data.categoryCd == 'CATE-PERSNAL-01' 
                  ? <Chip label="지원/신청" className='wh'/> 
                  : <Chip label="데이터/시스템" className='wh'/>
                  : null
                }
              </Stack>
              ...
              <Fragment>
                ...
                <Box css={styles.qna_box}>
                  <dl>
                    <dt>답변</dt>
                    <dd>
                      <div>{!!data ? !!data.answer ? data.answer.replace(/(?:\r\n|\r|\n)/g, '\r\n').split('\r\n').map((item:any) => ( <p>{item} </p>)) : '' : ''}</div>
                      <span className="date">
