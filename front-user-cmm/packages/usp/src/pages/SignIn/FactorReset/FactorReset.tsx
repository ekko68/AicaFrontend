...
function Factor() {
  ...
  const [values, setValues] = useState<PwState>({
    amount: "",
    password: "",
    passOneRest: "",
    weight: "",
  ...
  const handleSubmitClick = async (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (!!receive.state?.secessionKey) {
      await fetchAccountUnlocck({
        passwd1: values.password,
        passwd2: values.passOneRest,
        sessionId: receive.state?.secessionKey
      }).then((res: any) => {
        navigate('/signup/dormancyLift')
      }).catch((e) => {
        console.log('err - ', e.response.message)
        alert("일치하는 회원정보가 없습니다." + e.message)
      });
      return;
    }

    await fetchFactorPwChange({passwd1: values.password, passwd2: values.passOneRest, key: key}).then((res: any) => {
      ...
      <div css={styles.content}>
        ...
        <Stack spacing={2} direction="row" justifyContent="center" css={styles.signbtn}>
          <Button fullWidth variant="contained" type="button" className="primary"
                  onClick={handleSubmitClick}>확인</Button>
        </Stack>
      </div>
    </section>
  );
